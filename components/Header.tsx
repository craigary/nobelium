import { MouseEventHandler, MouseEvent, ReactNode, forwardRef, useCallback, useEffect, useRef, useState } from 'react'
...
export default function Header ({ navBarTitle, fullWidth }: { navBarTitle?: ReactNode; fullWidth?: boolean }) {
  const BLOG = useConfig()
  const { dark } = useTheme()
  ...
  const handler: IntersectionObserverCallback = useCallback(([entry]) => {
    if (navRef.current) {
      if (useSticky) {
  ...
  const handleClickHeader: MouseEventHandler<HTMLDivElement> = (ev) => {
    if (navRef.current !== ev.target && titleRef.current !== ev.target) return
...
const HeaderName = forwardRef<HTMLParagraphElement | null, HeaderNameProps>(function HeaderName ({ siteTitle, siteDescription, postTitle, onClick }, ref) {
  return (
    <p
      ref={ref}
