        ...
        {Object.keys(tags).map(key => {
          ...
          return (
            <Link key={key} href={selected ? '/search' : `/tag/${encodeURIComponent(key)}`}>
              <a>
                <li
                  className={`mr-3 py-2 font-medium border px-4 whitespace-nowrap  ${
                    selected
                      ? 'text-white bg-black border-black dark:bg-gray-600 dark:border-gray-600 dark:text-gray-300'
                      : 'bg-gray-100 border-gray-100 text-gray-400 dark:bg-night dark:border-gray-800 dark:text-gray-300'
                  }`}
                >
                  {`${key} (${tags[key]})`}
                </li>
