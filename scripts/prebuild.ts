...
void async function main () {
  ...
  if (!fs.existsSync(resolve(ROOT, 'blog.config.js'))) {
    ...
    if (!typeProp) {
      console.error('Cannot find a `type` prop!')
      abort()
    }
    ...
    if (!configRecord) {
      console.error('Cannot find a remote config!')
      abort()
    }
    ...
    if (!configCodeBlockId) {
      console.error('Cannot find a code block!')
      abort()
    }
    ...
    const config = (() => {
      try {
        ...
        console.error('The content of the first code block is not a valid JSON!')
        abort()
      }
    })()
